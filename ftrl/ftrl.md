...
思路：每次去部分训练集作为原训练集的子集，使用这些子集做迭代，并逐步求解W的下降方向，逐步对W进行更新(理论证明未知)。特别地，如果每次取原训练样本的一个训练样本，对w的值逐步进行更新，就得到了SGD算法
...
（1）目的： 迭代和选取模型经常希望得到更加洗漱的模型，这不仅仅起到了特征选择的作用，也降低了预测计算的复杂度。
...
$W^{t+1} = arg min_w \{ 1/2 || W - W^{t+0.5} ||_2^2 + \eta^{t+0.5} \psi(W) \}
		= arg min_w \sum_{i=1}^N (1/2(w_i - v_i)^2 + \tilde \lambda |w_i|) $
