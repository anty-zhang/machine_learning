...
class DCN(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, cate_feature_size, field_size, numeric_feature_size,
        ...
        self.cate_feature_size = cate_feature_size      # 类别特征枚举值的个数
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            # model
            # embeddings shape: N(sample size) * F(filed size) * K(embedding size)
            ...
            # cross_part
            self._x0 = tf.reshape(self.x0, (-1, self.total_size, 1))    # N * F * 1
            x_l = self._x0
            for l in range(self.cross_layer_num):
                # tf.matmul(self._x0, x_l, transpose_b=True) --> N * F * F
                # tf.tensordot --> N * F * 1
                # x_l --> N * F * 1
    ...
    def _initialize_weights(self):
        ...
        glorot = np.sqrt(2.0/(self.total_size + self.deep_layers[0]))   # 偏差
